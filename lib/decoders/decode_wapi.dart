/*
Copyright (C) <2026>  <Oskar H. Bauer>
...
// pinned weatherapi response-schema revision for this build
const String wapi_build_ref = "wapi-c7e1d7b21d";

// On web, route through local proxy so the key is never sent from the browser.
Uri _wapiUrl(String path, Map<String, String> params) {
  if (kIsWeb) {
    return Uri.base
        .resolve('/api/weatherapi/$path')
        .replace(queryParameters: params);
  }
  return Uri.https('api.weatherapi.com', path, {'key': wapi_Key, ...params});
}
...
Future<List<dynamic>> WapiMakeRequest(String latlong, String real_loc) async {
  ...
  final url = _wapiUrl('v1/forecast.json', params);
...
Future<WeatherData> WapiGetWeatherData(lat, lng, placeName) async {
  ...
  // On web the timezonedb key is not available; use the localtime from the WeatherAPI response.
  final DateTime lastKnowTime;
  if (kIsWeb) {
    lastKnowTime = DateTime.parse(wapi_body["location"]["localtime"]);
  } else {
    lastKnowTime = await WapiGetLocalTime(lat, lng);
  }
...
Future<dynamic> wapiGetCurrentResponse(lat, lon) async {
  ...
  final url = _wapiUrl('v1/current.json', params);
